...
class PersonalBrainRuntime:
    ...
    def process_sources(self, sources: list[dict[str, Any]]) -> dict[str, int]:
        ...
        for item in sources:
            ...
            normalized_records = [self._build_record(source, parsed_record) for parsed_record in parsed_records]
            for rec in normalized_records:
                record_rows[rec["record_id"]] = rec

            parsed_entities = parser.extract_entities(normalized_records, source)
            normalized_entities = [self._build_entity(source, parsed_entity, normalized_records) for parsed_entity in parsed_entities]
            for ent in normalized_entities:
                entity_rows[ent["entity_id"]] = ent

            parsed_relations = parser.build_relations(normalized_records, normalized_entities, source)
            normalized_relations = [self._build_relation(source, rel, normalized_records, normalized_entities) for rel in parsed_relations]
    ...
    def _build_relation(self, source: dict, relation: dict, records: list[dict], entities: list[dict]) -> dict:
        subject_name = relation.get("subject", "unknown")
        object_name = relation.get("object", "unknown")
        subject_id = entity_id("topic", subject_name)
        object_id = entity_id("topic", object_name)
        evidence = [r["record_id"] for r in records if relation.get("evidence_external_id", "") in {r.get("external_id", "")}] or [r["record_id"] for r in records[:1]]
        rid = relation_id(subject_id, relation.get("predicate", "relates_to"), object_id, evidence)
        return {
            "relation_id": rid,
