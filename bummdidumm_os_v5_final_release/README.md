...
- **deploy.sh** nutzt ausschließlich Environment Variablen (`PROJECT_ID`, `TARGET_FOLDER_ID`, `BRAIN_INDEX_ROOT`, `ARCHIVE_FOLDER_ID`, `INDEX_FOLDER_ID`, `CONTROL_SHEET_ID`, `GEMINI_API_KEY`).
- `TARGET_FOLDER_ID` ist zwingend; leer bedeutet Abbruch (Fail-Fast).
- `BRAIN_INDEX_ROOT` ist zwingend (z.B. `/tmp/brain_index` für Cloud Run Ephemeral).
