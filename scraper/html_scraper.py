...
def fetch_source_soup(source: dict):
    ...
    fetched = fetch_html(source["url"], source_id=source["id"], prefer_dynamic=force_dynamic, prefer_stealth=force_stealth)
    if fetched.get("soup"):
        return fetched
    ...
    if fetched.get("status_code") in {404, 410}:
        ...
        if relocated:
            ...
            fetched = fetch_html(relocated, source_id=source["id"], prefer_dynamic=True, prefer_stealth=force_stealth)
    return fetched
...
def scrape_source(source):
    ...
    print(f"[{source['id']}] {len(clean)} Events (mode={fetched.get('mode')})")
    return clean
